...
def popup():
    messagebox.showerror("You clicked a menu item! ", "Good job! Or is it ...");

def something():
    messagebox.showwarning("Maybe something happened", "Maybe nothing did ...")
...
menuOne.add_command(label="Open", command=fileExpl)
menuTwo.add_command(label="Whatever", command=popup)
menuTwo.add_command(label="Color ?", command=ColorPick)
menuOne.add_command(label="Konec", command=root.quit) 
menuThree.add_command(label="Something", command=something)
...
root.iconbitmap("C:/Users/marti/Documents/VSCode/School/URO/Labs/lab04/book.ico")
